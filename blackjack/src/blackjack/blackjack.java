...
for(int j=0;j<numero;j++){
    if((resultados[0]==2)||(resultados[0]==1)) saldos[j] = saldos[j] - manoJugador[j].apuesta;
    else{
        switch (resultados[j+1]){
            case -1: saldos[j] = saldos[j] - manoJugador[j].apuesta;break;
            case 1: saldos[j] = saldos[j] + manoJugador[j].apuesta;break;
            case 2: saldos[j] = saldos[j] + 2*manoJugador[j].apuesta;break;
...
break;                  
 ...
 // Si no se planta seguimos con una nueva carta
            ...
            TextIO.putln("Y usted tiene " + manoJugador[j].getBlackjackValor()+" puntos");
...
// Si se pasa de 21 puntos pierde y pone su resultado a -1
 ...
 while(true){
  ...
  for(i=0;i<jugadores;i++){
      if(manoBanca.getBlackjackValor()>=manoJugador[i].getBlackjackValor())beneficio=beneficio+manoJugador[i].apuesta;
        else if(resultados[i+1]!=-1)beneficio=beneficio-manoJugador[i].apuesta;   
  }
...
// posibles cartas y en funcion de eso el beneficio hipotetico que obtendriamos. 
  ...
  for (j=1;j<14;j++){ 
   ...
   for(i=0;i<jugadores;i++){ 
    ...
    if(manoBanca.getBlackjackValor()>=manoJugador[i].getBlackjackValor()) beneficio_aux=beneficio_aux+manoJugador[i].apuesta;
    else if(resultados[i+1]!=-1) beneficio_aux=beneficio_aux-manoJugador[i].apuesta;  
   }
...
// Si el beneficio hipotetico es mayor que el actual robamos carta 
 ...
 // Vamos a comparar los puntos de cada uno para ver quien ha ganado a quien y lo reflejamos   
// en el array de resultados 
 ...
 if(manoJugador[i].getBlackjackValor()>manoBanca.getBlackjackValor())resultados[i+1]=1;
 ...
 // Este metodo comprueba si todos los jugadores se han quedado sin dinero
